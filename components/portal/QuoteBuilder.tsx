...
export default function QuoteBuilder({
  ...
  // Вступний і завершальний текст друкованого документа — окремі під
  // кожну категорію, бо "сподіваємось на довгу співпрацю" звучить дивно
  // для роздрібного клієнта, який купує двері собі додому, а не стає
  // бізнес-партнером IN WOOD.
  const TARIFF_MESSAGES: Record<Tariff, { intro: string; closing: string }> = {
    retail: {
      intro: "Компанія IN WOOD рада запропонувати Вам двері та супутні матеріали, виготовлені з дотриманням найвищих стандартів якості.",
      closing: "Дякуємо, що обираєте IN WOOD! Завжди готові відповісти на Ваші запитання та допомогти з вибором.",
    },
    dealer: {
      intro: "Компанія IN WOOD рада запропонувати Вам комерційну пропозицію в напрямку виробництва та реалізації міжкімнатних дверей. Ми впевнені, що наша співпраця стане вигідною, тривалою та приємною.",
      closing: "Сподіваємось побачити Вас в числі наших партнерів і впевнені, що співпраця з IN WOOD буде вигідною, тривалою та приємною!",
    },
    distributor: {
      intro: "Компанія IN WOOD рада запропонувати Вам комерційну пропозицію для розвитку дистриб'юторської співпраці в напрямку міжкімнатних дверей та фурнітури.",
      closing: "Сподіваємось на довгострокову дистриб'юторську співпрацю та взаємовигідне партнерство з IN WOOD!",
    },
    builder: {
      intro: "Компанія IN WOOD рада запропонувати Вам комерційну пропозицію для комплектації Вашого будівельного об'єкта міжкімнатними дверима.",
      closing: "Будемо раді стати надійним постачальником дверей для Ваших будівельних проєктів і сподіваємось на подальшу співпрацю!",
    },
    epicenter: {
      intro: "Компанія IN WOOD рада запропонувати комерційну пропозицію в напрямку реалізації міжкімнатних дверей.",
      closing: "Дякуємо за співпрацю та сподіваємось на її подальший розвиток!",
    },
    export: {
      intro: "Компанія IN WOOD рада запропонувати Вам комерційну пропозицію щодо поставок міжкімнатних дверей на експорт.",
      closing: "Сподіваємось на довготривалу експортну співпрацю з IN WOOD і розвиток партнерства на міжнародному ринку!",
    },
  };
  ...
  function buildDocumentHtml() {
    ...
    const { intro, closing } = TARIFF_MESSAGES[tariff as Tariff] ?? TARIFF_MESSAGES.dealer;
    const introText = tt(intro);
    const closingText = tt(closing);
